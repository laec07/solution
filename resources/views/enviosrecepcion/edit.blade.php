...
<div class="card">
	<div class="card-header">
		<h4>Detalle envíos recepción</h4>
		<div class="row">
			<div class="col">
	...
	<div class="card-footer">
		...
		<a href="{{url('enviosrecepcion')}}" class="btn btn-secondary btn-block">Cancelar</a>
	</div>
</div>
...
function validabutton() {
	...
	if (a>0) {
				$("#button_g").html('<button title="Tiene guias pendientes de recibido " class="btn btn-primary  btn-block" disabled>Guardar</button>');
	}else{
		$("#button_g").html('<a href=" {{ url('/enviosrecepcion/'.$result->no_envio.'/estado') }} " class="btn btn-primary btn-block">Guardar</a>');
	}
}
